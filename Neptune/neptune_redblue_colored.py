...
Regulars = np.array([801, 802])
NumIrrs = np.arange(809, 813+1)
ProvIrrs = np.arange(85051, 85052+1)
...
### Planet as a sphere ###
r_planet = 0.00016553712  # Planet radius in AU
...
if DARK_MODE:
    ...
    fig.suptitle(f"Neptunian irregular moons ({END_DATE})", fontsize=suptitle_fs, color='white')
else:
    fig.suptitle(f"Neptunian irregular moons ({END_DATE})", fontsize=suptitle_fs)
